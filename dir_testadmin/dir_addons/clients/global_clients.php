...
class global_clients extends _global_content {
  ...
  public function install_plugin($group_id,$member_id) {
		if(!empty($group_id) && !empty($member_id)) {
			...
			if ($stmt->rowCount() == 1) {
				...
				$stmt = $this->pdo->prepare("SELECT * FROM connections WHERE connection_type='groupplugin' AND group_id='".$group_id."' AND plugin_id='".$plugin_data->plugin_id."'"); $stmt->execute();
				if ($stmt->rowCount() == 0) {
					$input_array['connection_type'] = "groupplugin";
					$input_array['member_id'] = $member_id;
					$input_array['group_id'] = $group_id;
					$input_array['plugin_id'] = $plugin_data->plugin_id;
					if(!$this->_add_insert("connections", $input_array)) { return "Error: [".basename(__FILE__)."] [".__LINE__."]"; }
  ...
  public function set_query($prefix="",$item="") {
    ...
    // START QUERY
    $sql = "SELECT * FROM content WHERE plugin_id=? AND group_id=? ";
