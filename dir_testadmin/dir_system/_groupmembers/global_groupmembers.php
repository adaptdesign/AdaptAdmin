...
class global_groupmembers extends _global_connections {
  ...
  public function set_query($prefix="M.",$item="") {
    ...
    // START QUERY
    $sql = "
    SELECT M.status, M.approval, M.level, M.connection_id, M.member_id AS member_id, U.firstname, U.lastname, U.profile_img, U.member_hash, U.last_active, M.created_date
    FROM connections M
    LEFT JOIN members U ON (M.member_id = U.member_id)
    WHERE M.connection_type='groupmember' AND M.group_id=? ";
