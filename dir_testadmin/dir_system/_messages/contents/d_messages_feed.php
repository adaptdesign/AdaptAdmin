...
class d_messages_feed extends global_messages {
    ...
    if(!empty($this->fail_page)) { output_error($this->fail_page); return; } //==============
      ...
      <ul class="ul-unstyled ul-inline clearfix relative uppercase f-w-500">
        ...
        <li class="right p-l-15">
          <a class="do_ajax btn btn-themed" da_type="load" da_target="doc_editor_content" data-postdata="none" da_link="<?=URL_PATH?>?page=<?=encode("d_messages_addedit")?>&action=<?=encode("p_addedit")?>&widg=<?=encode($this->widget_data['layout_id'])?>&link=<?=encode($this->link)?>">ADD <?=$this->widget_data['title']?></a>
        </li>
        <li class="right p-l-15 select2-b-p-0" style="min-width:150px">
          <?
    			$f_perpage["targets"] = array("feedlooper_messages");
    			$this->_show_filters_perpage($f_perpage); ?>
        </li>
        <li class="fill">
          <?
    			$f_main["targets"] = array("feedlooper_messages");
			...
			<div class="row equal m-0 m-t-15">
				<div class="col-md-12 p-15 p-t-0 p-r-0">
					...
					<div class="feedlooper" id="feedlooper_messages" page="1"  link="<?=URL_PATH?>?page=<?=encode("d_messages_feed")?>&action=<?=encode('p_feed_query')?>&widg=<?=encode($this->widget_data['layout_id'])?>&link=<?=encode($this->link)?>">

					</div>
					<div id="feedlooper_messages_msgbox" class="white"></div>
				</div>
			</div>
		</div>
		<? echo $this->feed_js(); ?>
