...
class global_groupsettings extends _global_groups {
  ...
  //=========================================
	public function install_plugin($group_id,$member_id) {
        ...
        // ADD PAGE WIDGETS
        $temp_plug =  array("layout");
        foreach($temp_plug as $row) {
          if($row=="groupsettings") {$tt="Details";} else {$tt=$row;}
          $stmt = $this->pdo->prepare("SELECT * FROM plugins WHERE title='".$row."'"); $stmt->execute();
    			if ($stmt->rowCount() == 1) {
    				$temp_plugin_data = $stmt->fetch(PDO::FETCH_OBJ);
            unset($input_array);
            $stmt = $this->pdo->prepare("SELECT * FROM layout WHERE layout_type='widg_page_page' AND group_id='".$group_id."' AND plugin_id='".$temp_plugin_data->plugin_id."' AND parent_id='".$plugin_data->plugin_id."'"); $stmt->execute();
    				if ($stmt->rowCount() == 0) {
    					$input_array['layout_type'] = "widg_page_page";
    					$input_array['member_id'] = $member_id;
    					$input_array['group_id'] = $group_id;
