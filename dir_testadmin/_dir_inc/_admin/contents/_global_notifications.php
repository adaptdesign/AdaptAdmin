...
class _global_notifications extends _global_admin {
  public $dbtype_inputs = array("notification_id","status","level","member_id","group_id","plugin_id","content_id","viewed","to_member_id","message","link","edit_date","created_date");
  ...
  public function _set_filters_dbtype($prefix="",$item="") {
    ...
    // IF SINGLE ITEM
    if(!empty($item) && is_numeric($item)){ $sql .= "AND ".$prefix."notification_id=? "; $arg[] = $item; }
    // RETURN ARRAY
