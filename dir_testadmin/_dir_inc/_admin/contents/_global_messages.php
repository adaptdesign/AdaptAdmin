...
class _global_messages extends _global_admin {
  public $dbtype_inputs = array("message_id","status","level","group_id","viewed","message_type","member_id","to_member_id","linked_to","message","edit_date","created_date");
  ...
  public function _set_filters_dbtype($prefix="",$item="") {
    ...
    // IF SINGLE ITEM
    if(!empty($item) && is_numeric($item)){ $sql .= "AND ".$prefix."message_id=? "; $arg[] = $item; }
    // RETURN ARRAY
