...
class _global_members extends _global_admin {
  public $dbtype_inputs = array("member_id","status","member_level","approval","member_hash","facebook_id","stripe_id","group_id","firstname","lastname","email","password","profile","profile_img","sessiontoken","authcode","count_mes","count_not","last_active","edit_date","created_date","time_zone","split_right_selection");
  ...
  public function _set_filters_dbtype($prefix="",$item="") {
    ...
    // IF SINGLE ITEM
    if(!empty($item) && is_numeric($item)){ $sql .= "AND ".$prefix."member_id=? "; $arg[] = $item; }
    // RETURN ARRAY
