...
class _global_layout extends _global_admin {
  public $dbtype_inputs = array("layout_id","status","level","order_id","member_id","group_id","plugin_id","parent_id","edit_date","created_date","layout_type","title","other");
  ...
  public function _set_filters_dbtype($prefix="",$item="") {
    ...
    // IF SINGLE ITEM
    if(!empty($item) && is_numeric($item)){ $sql .= "AND ".$prefix."layout_id=? "; $arg[] = $item; }
    // RETURN ARRAY
