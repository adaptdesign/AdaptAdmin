...
class _global_activities extends _global_admin {
  public $dbtype_inputs = array("activity_id","status","level","member_id","group_id","plugin_id","content_id","linked_to","message","link","edit_date","created_date");
  ...
  public function _set_filters_dbtype($prefix="",$item="") {
    ...
    // IF SINGLE ITEM
    if(!empty($item) && is_numeric($item)){ $sql .= "AND ".$prefix."activity_id=? "; $arg[] = $item; }
    // RETURN ARRAY
